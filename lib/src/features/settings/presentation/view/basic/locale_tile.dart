...
class LocaleTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return KListTile(
      leading: AnimatedWidgetShower(size: 30.0, child: AssetHelper.createSvgAsset(assetPath: SvgAssets.language)),
      ...
      trailing: OutlinedButton.icon(
        ...
        onPressed: () async =>
            await showDialog(context: context, barrierDismissible: false, builder: (_) => const LocaleChangerPopup()),
        label: Icon(Icons.arrow_forward_ios_rounded, size: 15.0, color: context.theme.primaryColor),
        icon: Consumer(
          builder: (_, ref, __) {
            final locale = ref.watch(localeProvider);
            return Text(locale.label, style: context.text.bodySmall!.copyWith(fontSize: 13.0, color: Colors.black));
          },
        ),
      ),
    );
              ...
              ...List.generate(
                ...
                (index) => Consumer(
                  builder: (_, ref, __) => KListTile(
                    onTap: () async =>
                        await ref.read(localeProvider.notifier).changeLocale(LocaleProfile.values[index]).then((_) {
                    ...
                    leading: Radio<LocaleProfile?>(
                      ...
                      onChanged: (v) async => await ref.read(localeProvider.notifier).changeLocale(v!).then((_) {
                        if (!context.mounted) return;
                        context.pop();
                      }),
                    ),
